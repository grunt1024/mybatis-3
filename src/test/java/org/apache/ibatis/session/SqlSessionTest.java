...
public class SqlSessionTest extends BaseDataTest {
    ...
    public void shouldSelectAllAuthors() throws Exception {
        ...
        try {
            ...
            LogUtil.log("创建sqlSession, 执行查询语句 selectAllAuthors ");
    ...
    @Test
    public void shouldSelectAllAuthors2() throws Exception {
        SqlSession session = sqlMapper.openSession(TransactionIsolationLevel.SERIALIZABLE);
        try {
            //创建一个sqlSession,并且执行查询语句
            LogUtil.log("创建sqlSession, 执行查询语句 selectAllAuthors ");

            final AuthorMapper mapper = session.getMapper(AuthorMapper.class);
            final List<Author> authors = mapper.selectAllAuthors();
            assertEquals(2, authors.size());
        } finally {
            session.close();
        }
    ...
    public void shouldFailWithTooManyResultsException() {
        SqlSession session = sqlMapper.openSession(TransactionIsolationLevel.SERIALIZABLE);
        try {
            LogUtil.log("创建sqlSession, 执行查询一条数据方法, 传入的是查询多条数据的sql,会报错");
